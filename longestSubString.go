...
func longestSubtring(A string) int {
	n := len(A)
	m := make(map[rune]int)
	left := 0
	right := 0
	...
	for i, ele := range A {
		...
		if ok == false {
			m[ele] = i
			right++
			fmt.Println("U:", i, right-left, left, right, string(ele))
			...
			fmt.Println("NU:", i, right-left, left, right, string(ele))
			if left <= m[ele] {
				left = m[ele] + 1
			}
			right++
